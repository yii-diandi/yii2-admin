...
- Enh: Custom side menu via `diandi/admin\Module::items` (mdmunir).
...
- Chg: Can customize return of `diandi/admin\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `diandi/admin\components\BizRule` (mdmunir).
...
- Chg: Remove `diandi/admin\Module::allowActions`. Set access control directly with `diandi/admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `diandi/admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
